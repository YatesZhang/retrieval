""" 
    to do: 
    @dataset: def collate_fn_coco
    @this: 
        dataloader = iter(dataloader)
        prompter = cocoprompter

        merge dataload & prompter to one iter
...
def train_epoch(
    ...
    with tqdm(range(num_steps), desc=f"Epoch [{epoch + 1}]", disable=not enable_pbar) as pbar:
        for _ in pbar:
            loss, _ = run_forward_backward(model, optimizer, criterion, data_iter, booster, prompter)
            optimizer.step()
            lr_scheduler.step()
...
def main():
    ...
    model, clip_prossesor, sam_transforms = create_model_and_transforms(
    ...
    logger.info(f"Finish loading model from {}".format(sam_checkpoint_path), ranks=[0])
    logger.info("model: {}".format(str(model)), ranks=[0])
    ...
    train_dataset = build_COCO(IMAGES_PATH, ANNOTATIONS_PATH, sam_transforms=sam_transforms)
    prompter = COCOPrompter(ANNOTATIONS_PATH, img_dir=IMAGES_PATH, shot=5, mask_rate=0.25, coco=train_dataset.coco)
    #######################################################################################
    ...
    for epoch in range(args.num_epoch):
        train_epoch(epoch, model, optimizer, criterion, lr_scheduler, train_dataloader, booster, coordinator, prompter)
