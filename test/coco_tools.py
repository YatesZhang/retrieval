...
class COCOAttr(Dataset):
    def __init__(self, image_dir=img_dir, annoFile=annFile, out_dir='.') -> None:
        super().__init__()
        self.annoFile = annFile
        self.coco = COCO(annoFile)
        # 获取所有图像的ID
        image_ids = self.coco.getImgIds()
        instance_id = 0
        # 可视化每个图像
        for image_id in tqdm(image_ids, desc="image"):
            # 获取图像信息和标注
            ...
            for annotation in annotations:
                ...
                x2, y2 = min(bbox[0] + bbox[2], width - 1), min(bbox[1] + bbox[3], height - 1)
                try:
                ...
                instance_id += 1 
                # 绘制标签
                label = self.coco.loadCats(category_id)[0]['name'].replace(" ", '-')
                save_dir = osp.join(out_dir, label)
                if not osp.exists(save_dir):
                    os.mkdir(save_dir)
                cropped_image.save(osp.join(save_dir, f'{instance_id}.jpg'))
...
if __name__ == "__main__":
    data_root = "/home/yunzhi/yunzhi/yunzhi/VLLM/retrieval/test/data"
